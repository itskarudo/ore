...
namespace Ore::Parser {
	RDParser::RDParser(Ore::Parser::Lexer Scanner) : Scanner(Scanner) {
	...
	Ore::Parser::Token RDParser::ConsumeToken(Ore::Parser::Token::TokenType type, const std::string& ErrorMessage) {
		if (Check(type)) {
			return Advance();
		}
	...
	std::unique_ptr<AST::Program> RDParser::Parse() {
		...
		while(!isAtEnd()) {
			...
			if(!ErrorFound) Program->append(std::move(Statement)); else return std::make_unique<AST::Program>();
		}
